          ...
          <div class="border border-white/10 rounded-lg overflow-hidden">
            <table class="w-full">
              ...
              <tbody>
                ...
                @empty {
                  <tr><td colspan="6" class="text-white/40 text-center py-10">Aucune réservation.</td></tr>
                }
              </tbody>
            </table>
          </div>
...
export class ReservationsAdminComponent implements OnInit {
  ...
  approve(id: number): void {
    ...
    this.reservationService.approve(id).subscribe({
      next: () => { this.updateStatus(id, 'CONFIRMEE'); this.busy.set(null); },
  ...
  private updateStatus(id: number, statut: string): void {
    this.reservations.update(list => list.map(r => r.id === id ? { ...r, statut } : r));
  }
